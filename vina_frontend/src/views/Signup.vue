<template>
    <div class="w3-padding-small">
        ...
        <div class="main w3-padding-small">
            ...
            <form @submit.prevent="signupUser" autocomplete="on">
                <div class="error" v-if="error_message">
                    {{error_message}}
                </div>

                <div class="error" v-if="authError['signupError']">{{ authError['signupError'][0] }}<br/>
                </div>
                ...
                <router-link :to="{ name: 'Login' }" class="w3-bar-item">
  ...
  methods: {
    ...
    signupUser: function () {
      ...
      if (this.password != this.password2) {
        ...
        this.error_message = "Passwords does'nt match."        
      } else {
          this.signup(userInfo).then(res => {})
          .catch((err) => {
            this.isLoading = false
          })
      }
